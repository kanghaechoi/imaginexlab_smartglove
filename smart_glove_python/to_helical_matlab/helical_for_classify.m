    ...
    PROMPT_AGE = 'Insert age (Options: 20, 50, 70): ';
    AGE = input(PROMPT_AGE);
    
    hand_count = numel(dir(sprintf('../data/%s/Hand_IMU_%d_*.txt', QUESTION, AGE)));
    wrist_count = numel(dir(sprintf('../data/%s/Wrist_IMU_%d_*.txt', QUESTION, AGE)));
    ...
    for i=1:file_count
        % Read hand data
        if i < 10
            hand_data = readmatrix(sprintf('../data/%s/Hand_IMU_%d_0%d.txt', QUESTION, AGE, i));
            sprintf('../data/%s/Hand_IMU_%d_0%d.txt', QUESTION, AGE, i)
        else
            hand_data = readmatrix(sprintf('../data/%s/Hand_IMU_%d_%d.txt', QUESTION, AGE, i));
            sprintf('../data/%s/Hand_IMU_%d_%d.txt', QUESTION, AGE, i)
        end   

        % Read wrist
        if i < 10
            wrist_data = readmatrix(sprintf('../data/%s/Wrist_IMU_%d_0%d.txt',  QUESTION, AGE, i));
            sprintf('../data/%s/Wrist_IMU_%d_0%d.txt',  QUESTION, AGE, i)
        else
            wrist_data = readmatrix(sprintf('../data/%s/Wrist_IMU_%d_%d.txt',  QUESTION, AGE, i));
            sprintf('../data/%s/Wrist_IMU_%d_%d.txt',  QUESTION, AGE, i)
        end
        ...
        if i < 10
            writematrix(helical_data, sprintf('../data/%s/Helical_IMU_%d_0%d.txt', QUESTION, AGE, i));
            sprintf('../data/%s/Helical_IMU_%d_0%d.txt', QUESTION, AGE, i)
        else
            writematrix(helical_data, sprintf('../data/%s/Helical_IMU_%d_%d.txt', QUESTION, AGE, i));
            sprintf('../data/%s/Helical_IMU_%d_%d.txt', QUESTION, AGE, i)
        end
