...
def feature_extraction(hand_data, wrist_data):
    ...
    '''
    scaled_thumb_acc_mag = featureScale(thumb_acc_mag)
    scaled_index_acc_mag = featureScale(index_acc_mag)
    scaled_wrist_acc_mag = featureScale(wrist_acc_mag)

    scaledThumbAngleX = featureScale(np.deg2rad(thumb_x_angle))
    scaledIndexAngleX = featureScale(np.deg2rad(index_x_angle))
    scaledHandAngleX = featureScale(np.deg2rad(hand_x_angle))
    scaledHandAngleY = featureScale(np.deg2rad(hand_y_angle))
    scaledHandAngleZ = featureScale(np.deg2rad(hand_z_angle))
    scaledWristAngleX = featureScale(np.deg2rad(wrist_x_angle))
    scaledWristAngleY = featureScale(np.deg2rad(wrist_y_angle))
    scaledWristAngleZ = featureScale(np.deg2rad(wrist_z_angle))
    ...
    return [resized_thumb_acc_mag,
            resized_index_acc_mag,
            resized_wrist_acc_mag,
            resized_thumb_x_angle,
            resized_index_x_angle,
            resized_hand_x_angle,
            resized_hand_y_angle,
            resized_hand_z_angle,
            resized_wrist_x_angle,
            resized_wrist_y_angle,
...
class UserDataframe:
    def __init__(self, hand_data, wrist_data, age):
        self.hand_data = hand_data
        self.wrist_data = wrist_data
        self.features, self.length = feature_extractionDf(hand_data, wrist_data)
        self.labels = age / 10
...
def feature_extractionDf(hand_data, wrist_data):
    feature_length = len(hand_data)
    if len(hand_data) > len(wrist_data):
        feature_length = len(wrist_data)
    ...
    d = {'resized_thumb_acc_mag': resized_thumb_acc_mag,
         'resized_index_acc_mag': resized_index_acc_mag,
         'resized_wrist_acc_mag': resized_wrist_acc_mag,
         'resized_thumb_x_angle': resized_thumb_x_angle,
         'resized_index_x_angle': resized_index_x_angle,
         'resized_hand_x_angle': resized_hand_x_angle,
         'resized_hand_y_angle': resized_hand_y_angle,
         'resized_hand_z_angle': resized_hand_z_angle,
         'resized_wrist_x_angle': resized_wrist_x_angle,
         'resized_wrist_y_angle': resized_wrist_y_angle,
         'resized_wrist_z_angle': resized_wrist_z_angle}
...
def featureScale(inputColumn):
    # Feature scaling
    meanColumn = inputColumn.mean();  # Mean
    stdColumn = inputColumn.std();  # Standard deviation
    scaledFeature = (inputColumn - meanColumn) / stdColumn;  # (Variables - Mean) / Stdend
