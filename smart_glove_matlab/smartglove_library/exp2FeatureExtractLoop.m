function [features, labels] = exp2FeatureExtractLoop(B1, A1, AGE, fileCount, GENDER, subjectNum, gestureID, numberOfFeatures)
%% Feature extraction

handData = dlmread(sprintf('Hand_IMU_%d_%d_%d_0%d_0%d.txt', AGE, GENDER, subjectNum, gestureID, fileCount)); %Read hand data
wristData = dlmread(sprintf('Wrist_IMU_%d_%d_%d_0%d_0%d.txt', AGE, GENDER, subjectNum, gestureID, fileCount)); %Read wrist
...
smartPhone = [resizedWristAngleZ, resizedWristAccY, resizedThumbAngleX, ...
    resizedHandAngleY, resizedHandAngleZ, resizedIndexAccY, ...
    resizedWristVelX, resizedIndexVelX, resizedThumbVelZ, ...
    resizedThumbAccX, resizedIndexAccZ, resizedWristVelY, ...
    resizedThumbAccY, resizedWristVelZ, resizedWristAccZ, ...
    resizedIndexAngleX, resizedThumbVelY, resizedIndexVelZ, ...
    resizedWristAngleX, resizedThumbAccZ, resizedThumbVelX, ...
    resizedIndexVelY, resizedWristAngleY, resizedWristAccX, ... 
    resizedHandAngleX, resizedIndexAccX];

carKey = [resizedWristAngleZ, resizedWristAngleX, resizedWristAccY, ...
    resizedWristVelX, resizedWristAccZ, resizedThumbAngleX, ...
    resizedWristVelY, resizedThumbVelZ, resizedWristAccX, ...
    resizedWristVelZ, resizedIndexVelX, resizedWristAngleY, ...
    resizedHandAngleZ, resizedIndexAngleX, resizedHandAngleX, ...
    resizedIndexVelZ, resizedThumbAccY, resizedThumbVelY, ...
    resizedIndexVelY, resizedThumbAccZ, resizedIndexAccY, ...
    resizedThumbVelX, resizedHandAngleY, resizedThumbAccX, ... 
    resizedIndexAccX, resizedIndexAccZ];

espressoCup = [resizedWristAngleZ, resizedWristAngleX, resizedWristAccY, ...
    resizedWristVelY, resizedIndexAccZ, resizedIndexAngleX, ...
    resizedHandAngleY, resizedThumbVelZ, resizedIndexAccY, ...
    resizedThumbAccX, resizedWristVelZ, resizedWristVelX, ...
    resizedThumbVelX, resizedThumbVelY, resizedIndexVelX, ...
    resizedWristAccZ, resizedThumbAccZ, resizedThumbAngleX, ...
    resizedThumbAccY, resizedIndexVelZ, resizedWristAngleY, ...
    resizedWristAccX, resizedHandAngleX, resizedIndexAccX, ...
    resizedHandAngleZ, resizedIndexVelY];

pillBottle = [resizedWristAngleZ, resizedIndexAngleX, resizedWristAngleX, ...
    resizedHandAngleZ, resizedWristAngleY, resizedIndexVelZ, ...
    resizedWristAccY, resizedThumbAccX, resizedThumbVelY, ...
    resizedWristAccX, resizedWristAccZ, resizedIndexAccY, ...
    resizedThumbVelZ, resizedThumbAngleX, resizedWristVelZ, ...
    resizedHandAngleY, resizedIndexAccZ, resizedThumbAccY, ...
    resizedWristVelY, resizedIndexAccX, resizedHandAngleX, ...
    resizedThumbVelX, resizedWristVelX, resizedIndexVelX, ...
    resizedIndexVelY, resizedThumbAccZ];

sodaCan = [resizedIndexAngleX, resizedWristVelY, resizedWristAccY, ...
    resizedWristAngleZ, resizedIndexAccY, resizedThumbAngleX, ...
    resizedHandAngleZ, resizedWristAngleY, resizedThumbAccX, ...
    resizedIndexAccZ, resizedIndexVelZ, resizedWristAccX, ...
    resizedThumbVelZ, resizedWristAngleX, resizedHandAngleX, ...
    resizedThumbVelY, resizedWristVelX, resizedWristAccZ, ...
    resizedIndexVelX, resizedIndexAccX, resizedThumbAccZ, ...
    resizedWristVelZ, resizedThumbVelX, resizedHandAngleY,...
    resizedThumbAccY, resizedIndexVelY];

features = sodaCan(:, 1:numberOfFeatures)';
%% Create a label column (number of files, so each roz of the cell array x 1) 

labels = ones(1, 1);
labels = ((GENDER * 100) + (subjectNum * 10) + gestureID) * labels;
