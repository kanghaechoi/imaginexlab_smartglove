function [features, labels] = feature_extraction_toCell(b1, a1, age, file_count)
%% Feature extraction
...
%% Feature scaling
%Acceleration
scaled_thumb_acc_mag = normalize(thumb_acc_mag, 'norm');
scaled_index_acc_mag = normalize(index_acc_mag, 'norm');
scaled_wrist_acc_mag = normalize(wrist_acc_mag, 'norm');

%Euler angle
scaled_thumb_x_angle = normalize(thumb_x_angle, 'norm');
scaled_index_x_angle = normalize(index_x_angle, 'norm');
scaled_hand_x_angle = normalize(hand_x_angle, 'norm');
scaled_hand_y_angle = normalize(hand_y_angle, 'norm');
scaled_hand_z_angle = normalize(hand_z_angle, 'norm');
scaled_wrist_x_angle = normalize(wrist_x_angle, 'norm');
scaled_wrist_y_angle = normalize(wrist_y_angle, 'norm');
scaled_wrist_z_angle = normalize(wrist_z_angle, 'norm');

%% Data filtering

%Acceleration
zpl_thumb_acc_mag = (filtfilt(b1,a1,scaled_thumb_acc_mag'))'; %Zero-phase filtering + Matrix transpose
zpl_index_acc_mag = (filtfilt(b1,a1,scaled_index_acc_mag'))'; %Zero-phase filtering + Matrix transpose
zpl_wrist_acc_mag = (filtfilt(b1,a1,scaled_wrist_acc_mag'))'; %Zero-phase filtering + Matrix transpose

%Euler angle
zpl_thumb_x_angle = (filtfilt(b1,a1,scaled_thumb_x_angle'))'; %Zero-phase filtering + Matrix transpose
zpl_index_x_angle = (filtfilt(b1,a1,scaled_index_x_angle'))'; %Zero-phase filtering + Matrix transpose
zpl_hand_x_angle = (filtfilt(b1,a1,scaled_hand_x_angle'))'; %Zero-phase filtering + Matrix transpose
zpl_hand_y_angle = (filtfilt(b1,a1,scaled_hand_y_angle'))'; %Zero-phase filtering + Matrix transpose
zpl_hand_z_angle = (filtfilt(b1,a1,scaled_hand_z_angle'))';%Zero-phase filtering + Matrix transpose
zpl_wrist_x_angle = (filtfilt(b1,a1,scaled_wrist_x_angle'))'; %Zero-phase filtering + Matrix transpose
zpl_wrist_y_angle = (filtfilt(b1,a1,scaled_wrist_y_angle'))'; %Zero-phase filtering + Matrix transpose
zpl_wrist_z_angle = (filtfilt(b1,a1,scaled_wrist_z_angle'))'; %Zero-phase filtering + Matrix transpose
...
features_data = [resized_thumb_acc_mag, resized_index_acc_mag, resized_wrist_acc_mag, resized_thumb_x_angle, resized_index_x_angle, ...
    resized_hand_x_angle, resized_hand_y_angle, resized_hand_z_angle, resized_wrist_x_angle, resized_wrist_y_angle, resized_wrist_z_angle]';
